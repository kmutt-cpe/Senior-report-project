...
the price is around 8385 Baht per month as you can see from Figure~\ref*{fig:min_gcp_price}.
...
The evaluation result of the Siamese neural network model is better than the cosine similarity because the aim of this prediction of this project wants to classify the intent of both questions as similar or not similar. From the example, the model can classify intent which is not similar better than cosine similarity, and the model able to classify the similar intent of the question well to a certain extent but still not good as cosine similarity. In summary, As the aim of the prediction of the project, the model can both predict similar and not similar intent better than cosine similarity, because cosine similarity can't classify not similar intent. But the overall performance of the model is not good. The problem comes from not enough CPE questions data to train the Siamese neural network.
If we have more CPE questions data to train the Siamese neural network, we could improve the model's performance is not good. The problem comes from not enough CPE questions data to train the Siamese neural network. If we have more CPE questions data to train the Siamese neural network, we could improve the model's performance. Due to the problem of Timeout. The response time for replying answer to the model used is slower than cosine similarity. The project has to used cosine similarity, although the evaluation of the cosine result is less than the siamese neural network model. 
...
The evaluation result of Siamese neural network model is better than the cosine simiarity model. The model able to classify the intent of the question well to a certain extent but the response time for replying user of the model is slower than cosine similarity. Overall performance of the model is not good. The problem comes from not enough CPE questions data to train the Siamese neural network.
If we have more CPE questions data to train the Siamese neural network, we could improve the performance of the model.
