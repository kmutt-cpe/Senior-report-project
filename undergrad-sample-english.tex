...
The Python programming language is a high-level computer programming language %todo: (Keras: the Python deep learning API, 2015)
...
can also be used for many different types of programming. Without being limited to a particular
...
by transformation written for the rule %todo:(Johnson, 2019)
. Each rule might have multiple
...
manipulate these variables %todo:(GARBER, 2014)
...
Botnoi Consulting has started from Botnoi chatbot in 2016 in Line Bot Awards competition in Japan %todo: (Botnoi, 2020)
...
\subsection{Automated Self-Learning Chatbot Initially Built as a FAQs Database Information Retrieval System: Multi-level and Intelligent Universal Virtual Front-Office Implementing Neural Network}
This paper is about implement the multi-level virtual front office as you can see from the Figure \ref*{fig:ch3_lr_self_learning_chatbot}. 
\begin{figure}[h!]
  \centering
  \setlength{\fboxrule}{0.2mm}
  \setlength{\fboxsep}{0.5cm}
  \fbox{\includegraphics[width=14cm]{./img/ch3/literature_review_self_learning_chatbot.png}}
...
\begin{figure}[h!]
  ...
  \setlength{\fboxrule}{0.2mm}
  \setlength{\fboxsep}{0.5cm}
  \fbox{\includegraphics[width=10cm]{./img/ch3/literature_review_activity_diagram_chatbot.png}}
...
\begin{figure}[h!]
  \centering
  \fbox{\includegraphics[width=15cm]{./img/ch3/solution_compare.png}}
...
Figure ~\ref{fig:sequence diagram of chat message} illustrates the chat workflow starting from a question from questioner until reply answer in knowledge management back. When the questioner sends the message via Facebook Messenger, the message will be sent directly to Dialogflow.\\
...
\begin{figure}[!h]
	...
	\caption{Use case of KMUTT CPE Chatbot}\label{fig:sequence diagram of chat message}
\end{figure}

\pagebreak

\subsection{The sequence diagram of automate build model}
Figure ~\ref{fig:sequence diagram of automate build model} illustrates the workflow of automating build prediction models. Apache Airflow automatically gets the question from the back-end API, builds the model, and deploys on Google Cloud
\begin{figure}[!h]
	\includegraphics[width=14cm]{img/ch3/sequence diagram of automate build model.jpg}
	\caption{Use case of KMUTT CPE Chatbot}\label{fig:sequence diagram of automate build model}
\end{figure}
...
\textbf{predict\_task} - Message that going to predict in prediction model.\\
Used to keep log of question message and predict task.
\centering
\begin{tabular}{llp{0.6\linewidth}l}
\rowcolor[HTML]{5B9BD5} 
Attribute         & Type     & Description                                                                                                 \\
...
\textbf{fb\_message} - Message from Facebook webhook.\\
Used to keep log of question message and predict task.
\centering
\begin{tabular}{llp{0.6\linewidth}l}
\rowcolor[HTML]{5B9BD5} 
