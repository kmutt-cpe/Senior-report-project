...
The first approach we try to solve this problem is to increase the timeout of Dialogflow
...
the predicted question to return as you can see in Figure~\ref*{fig:model_pattern2}.

The result of the improving chat workflow is in Table~\ref*{tab:run_time_pattern2_table}.
Total time in each computer is not change too much from the previous one and
still do not have enough time to response to Dialogflow before timeout.
...
% todo: Show cost of the lowest budget for GCP price.
...
This is a FAQs of Computer Engineering questions which we gathering by ourself. Overall we have 177 questions we divided questions into 4 categories are Admission has 19 questions , FAQs has 36 questions, Enrollment has 40 questions and Curriculum has 82 questions then we use a deepcut to cut the word from the text we have read which is stored in a column called CPE questions. All data will be converted into a list of words in the CPE FAQs and change it to the bag of words (sparse matrix).
\begin{figure}[!h]\centering
...
For testing, in the example we input qustions to find what categories of this input question. We expected our model can predict this input question is in Curriculum category 
\begin{figure}[!h]\centering
...
\end{figure}
The model return all the value how the input question similar for each category and the we choose the max value to be catogry of the input question
\begin{figure}[!h]\centering
...
\end{figure}
\pagebreak
The max category is Curriculum which match with our expected result. So from the results we can predict categories quite accurately. 
